# Blaze2D WASM Examples

This folder contains examples and utilities for using the Blaze2D WASM module in web applications.
...
- **`blaze2d-wasm.d.ts`** - TypeScript type definitions for the WASM module
- **`useBandStructure.tsx`** - React hooks for band structure calculations
- **`bandWorker.ts`** - Web Worker utilities for background computation
...
```bash
# From the blaze2d root directory
make wasm
```

This creates `wasm-dist/` containing:
- `blaze2d_backend_wasm.js` - JavaScript glue code
- `blaze2d_backend_wasm_bg.wasm` - WebAssembly binary
- `blaze2d_backend_wasm.d.ts` - TypeScript definitions
...
```typescript
import init, { WasmBulkDriver } from '/wasm/blaze2d_backend_wasm.js';
...
| Hook | Threading | Best For |
|------|-----------|----------|
| `useBandStructureWorker()` | Web Worker (background) | **Real-time UI updates** (recommended) |
| `useBandStructureStreaming()` | Main thread | Testing, simple cases |
| `useBandStructure()` | Main thread | Parameter sweeps |

### Why Use Web Workers?

JavaScript is single-threaded. Without a Web Worker, the WASM computation
blocks the main thread, preventing React from updating the UI until the
entire computation is complete.

With `useBandStructureWorker()`:
- Computation runs in a background thread
- React can re-render after each k-point
- User can cancel the computation
