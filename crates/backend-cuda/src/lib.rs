...
mod gpu_field {
    ...
    fn fieldscalar_to_f32_slice_mut(data: &mut [FieldScalar]) -> &mut [f32] {
        unsafe { std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut f32, data.len() * 2) }
    }
    ...
    impl CudaField {
        ...
        pub fn from_host(stream: Arc<CudaStream>, grid: Grid2D, data: Vec<Complex64>) -> Self {
            ...
            let f64_data: Vec<f64> = data.iter().flat_map(|c| [c.re, c.im]).collect();
            let host_cache: Vec<FieldScalar> = data
                .iter()
...
impl SpectralBackend for CudaBackend {
    ...
    fn scale(&self, alpha: Complex64, buffer: &mut Self::Buffer) {
        use cudarc::cublas::sys::{cuDoubleComplex, cublasStatus_t, cublasZscal_v2};

        let n = buffer.len() as i32;
        let alpha_c = cuDoubleComplex {
            x: alpha.re,
            y: alpha.im,
        };
    ...
    fn axpy(&self, alpha: Complex64, x: &Self::Buffer, y: &mut Self::Buffer) {
        use cudarc::cublas::sys::{cuDoubleComplex, cublasStatus_t, cublasZaxpy_v2};
    ...
    fn dot(&self, x: &Self::Buffer, y: &Self::Buffer) -> Complex64 {
        use cudarc::cublas::sys::{cuDoubleComplex, cublasStatus_t, cublasZdotc_v2};
    ...
    fn gram_matrix(&self, x: &[Self::Buffer], y: &[Self::Buffer]) -> Vec<Complex64> {
        use cudarc::cublas::sys::{
            cuDoubleComplex, cublasOperation_t, cublasStatus_t, cublasZgemm_v2,
        };
        ...
        let x_f64 =
            unsafe { std::slice::from_raw_parts(x_host.as_ptr() as *const f64, x_host.len() * 2) };
        let y_f64 =
            unsafe { std::slice::from_raw_parts(y_host.as_ptr() as *const f64, y_host.len() * 2) };

        // Copy to device
        let x_dev = self
            .stream
            .clone_htod(x_f64)
            .expect("Failed to copy X to GPU");
        let y_dev = self
            .stream
            .clone_htod(y_f64)
            .expect("Failed to copy Y to GPU");

        // Allocate output matrix G (p × q) on device
        let mut g_dev = self
            .stream
            .alloc_zeros::<f64>(p * q * 2)
            .expect("Failed to allocate G on GPU");
        ...
        self.stream
            .memcpy_dtoh(&g_dev, &mut g_f64)
            .expect("Failed to copy G from GPU");
        ...
        let g_complex =
            unsafe { std::slice::from_raw_parts(g_f64.as_ptr() as *const Complex64, p * q) };
    ...
    fn linear_combinations(
        ...
        // Reinterpret as f64 for device copy
        let q_f64 =
            unsafe { std::slice::from_raw_parts(q_host.as_ptr() as *const f64, q_host.len() * 2) };
        let c_f64 =
            unsafe { std::slice::from_raw_parts(coeffs.as_ptr() as *const f64, coeffs.len() * 2) };
        ...
        let c_dev = self
            .stream
            .clone_htod(c_f64)
            .expect("Failed to copy C to GPU");

        // Allocate output matrix X (n × m) on device
        let mut x_dev = self
            .stream
            .alloc_zeros::<f64>(n * m * 2)
            .expect("Failed to allocate X on GPU");
        ...
        self.stream
            .memcpy_dtoh(&x_dev, &mut x_f64)
            .expect("Failed to copy X from GPU");

        // Reinterpret as Complex64 (column-major from GEMM)
        let x_complex =
            unsafe { std::slice::from_raw_parts(x_f64.as_ptr() as *const Complex64, n * m) };
        ...
        for j in 0..m {
            ...
            outputs.push(CudaField::from_host(
...
mod tests {
    ...
    fn test_gram_matrix() {
        ...
        assert!(
            (gram[0] - Complex64::new(1.0, 0.0)).norm() < 1e-10,
            "G[0,0] = {:?}",
            gram[0]
        );
        assert!(
            (gram[1] - Complex64::new(0.0, 1.0)).norm() < 1e-10,
            "G[0,1] = {:?}",
            gram[1]
        );
        assert!(
            (gram[2] - Complex64::new(0.0, 0.0)).norm() < 1e-10,
            "G[1,0] = {:?}",
            gram[2]
        );
        assert!(
            (gram[3] - Complex64::new(0.0, 0.0)).norm() < 1e-10,
            "G[1,1] = {:?}",
            gram[3]
        );
        assert!(
            (gram[4] - Complex64::new(s, 0.0)).norm() < 1e-10,
            "G[2,0] = {:?}",
            gram[4]
        );
        assert!(
            (gram[5] - Complex64::new(0.0, s)).norm() < 1e-10,
            "G[2,1] = {:?}",
            gram[5]
        );
    ...
    fn test_linear_combinations() {
        ...
        let coeffs = vec![
            ...
            Complex64::new(4.0, 0.0), // column 0
            Complex64::new(2.0, 0.0),
            Complex64::new(5.0, 0.0), // column 1
            Complex64::new(3.0, 0.0),
            Complex64::new(6.0, 0.0), // column 2
        ];
        ...
        assert!(
            (out0[0] - Complex64::new(1.0, 0.0)).norm() < 1e-10,
            "out0[0] = {:?}",
            out0[0]
        );
        assert!(
            (out0[1] - Complex64::new(4.0, 0.0)).norm() < 1e-10,
            "out0[1] = {:?}",
            out0[1]
        );
        for i in 2..16 {
            assert!(
                (out0[i] - Complex64::ZERO).norm() < 1e-10,
                "out0[{}] = {:?}",
                i,
                out0[i]
            );
        ...
        assert!(
            (out1[0] - Complex64::new(2.0, 0.0)).norm() < 1e-10,
            "out1[0] = {:?}",
            out1[0]
        );
        assert!(
            (out1[1] - Complex64::new(5.0, 0.0)).norm() < 1e-10,
            "out1[1] = {:?}",
            out1[1]
        );
        ...
        assert!(
            (out2[0] - Complex64::new(3.0, 0.0)).norm() < 1e-10,
            "out2[0] = {:?}",
            out2[0]
        );
        assert!(
            (out2[1] - Complex64::new(6.0, 0.0)).norm() < 1e-10,
            "out2[1] = {:?}",
            out2[1]
        );
    ...
    fn test_linear_combinations_complex() {
        ...
        assert!(
            (out[0] - expected0).norm() < 1e-10,
            "out[0] = {:?}, expected {:?}",
            out[0],
            expected0
        );
        assert!(
            (out[1] - expected1).norm() < 1e-10,
            "out[1] = {:?}, expected {:?}",
            out[1],
            expected1
        );
