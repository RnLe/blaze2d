...
exports.default = MemoryRatioChart;
...
function MemoryRatioChart({ width = 650, height = 380, }) {
    ...
    const { resolutionSeries, bandsSeries } = (0, react_1.useMemo)(() => {
        ...
        const computeRatio = (mpbVal, mpbStd, blazeVal, blazeStd) => {
            if (blazeVal === 0)
                return { ratio: 0, std: 0 };
        ...
        for (const val of resData.values) {
            ...
            if (mpbTM && blazeTM) {
                const { ratio, std } = computeRatio(mpbTM.memory_mb, mpbTM.memory_mb_std || 0, blazeTM.memory_mb, blazeTM.memory_mb_std || 0);
                resTM.push({ x: val, y: ratio, std: std > 0 ? std : undefined });
            }
            if (mpbTE && blazeTE) {
                const { ratio, std } = computeRatio(mpbTE.memory_mb, mpbTE.memory_mb_std || 0, blazeTE.memory_mb, blazeTE.memory_mb_std || 0);
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', flexDirection: 'column', gap: '0.5rem' }, children: [(0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', gap: '20px', justifyContent: 'center' }, children: [(0, jsx_runtime_1.jsx)(LineChart_1.default, { series: resolutionSeries, width: chartWidth, height: height, title: "Memory Efficiency Ratio vs Resolution", xLabel: "Resolution", yLabel: "Ratio (MPB / Blaze)", xTickFormat: (v) => `${v}`, yTickFormat: (v) => `${v.toFixed(0)}×`, showErrorBars: true, yDomain: [0, 35], margin: { top: 50, right: 20, bottom: 60, left: 70 } }), (0, jsx_runtime_1.jsx)(LineChart_1.default, { series: bandsSeries, width: chartWidth, height: height, title: "Memory Efficiency Ratio vs Bands", xLabel: "Number of Bands", yLabel: "Ratio (MPB / Blaze)", xTickFormat: (v) => `${v}`, yTickFormat: (v) => `${v.toFixed(0)}×`, showErrorBars: true, yDomain: [0, 25], margin: { top: 50, right: 20, bottom: 60, left: 70 } })] }), (0, jsx_runtime_1.jsx)("p", { style: {
                    marginTop: '1rem',
                    fontSize: '0.875rem',
                    color: '#888',
                    lineHeight: 1.5,
