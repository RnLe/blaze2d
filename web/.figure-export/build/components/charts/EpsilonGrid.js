...
function EpsilonGrid({ resolution, width = 300, height = 300, colormap = 'viridis', showColorbar = true, }) {
    const [data, setData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    // Validate resolution (must be 16-256, step 16)
    const validResolution = (0, react_1.useMemo)(() => {
    ...
    (0, react_1.useEffect)(() => {
        setLoading(true);
        setError(null);
        fetch((0, paths_1.getAssetPath)(`/data/epsilon-grids/epsilon-res${validResolution}.json`))
            .then(res => {
            if (!res.ok)
                throw new Error(`Failed to load epsilon grid (res=${validResolution})`);
            return res.json();
        })
            .then((jsonData) => {
            setData(jsonData);
    ...
    if (loading) {
        return ((0, jsx_runtime_1.jsx)("div", { style: { width, height, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f0f0' }, children: "Loading..." }));
    }
    if (error || !data || !imageDataUrl) {
        return ((0, jsx_runtime_1.jsxs)("div", { style: { width, height, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#ffe0e0', color: '#800' }, children: ["Error: ", error || 'Failed to render'] }));
    }
    const colorbarWidth = showColorbar ? 60 : 0;
    const imageWidth = width - colorbarWidth;
    const imageHeight = height;
    return ((0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', gap: '8px' }, children: [(0, jsx_runtime_1.jsx)("div", { style: { width: imageWidth, height: imageHeight }, children: (0, jsx_runtime_1.jsx)("img", { src: imageDataUrl, alt: `Smoothed epsilon grid (${data.resolution}×${data.resolution})`, style: {
                        width: '100%',
                        height: '100%',
                        imageRendering: 'pixelated',
                    } }) }), showColorbar && ((0, jsx_runtime_1.jsxs)("div", { style: { width: colorbarWidth - 8, display: 'flex', flexDirection: 'column', fontSize: '11px' }, children: [(0, jsx_runtime_1.jsxs)("div", { style: { flex: 1, display: 'flex', flexDirection: 'column' }, children: [(0, jsx_runtime_1.jsx)("span", { style: { textAlign: 'right', marginBottom: '2px' }, children: data.epsilon_background.toFixed(1) }), (0, jsx_runtime_1.jsx)("div", { style: {
                                    flex: 1,
                                    background: `linear-gradient(to bottom, ${Array.from({ length: 10 }, (_, i) => {
                                        const t = 1 - i / 9;
                                        const [r, g, b] = getColor(t, colormap);
                                        return `rgb(${r},${g},${b})`;
                                    }).join(', ')})`,
                                    borderRadius: '2px',
                                } }), (0, jsx_runtime_1.jsx)("span", { style: { textAlign: 'right', marginTop: '2px' }, children: data.epsilon_hole.toFixed(1) })] }), (0, jsx_runtime_1.jsx)("div", { style: { textAlign: 'center', marginTop: '4px', fontSize: '10px', color: '#666' }, children: "\u03B5" })] }))] }));
}
