...
def compare(reference, candidate):
    ...
    for name in ('frequencies', 'eigenvalues', 'k_points', 'k_points_cartesian', 'distances'):
        if name in reference:
            np.testing.assert_allclose(reference[name], candidate[name], rtol=5e-8, atol=5e-10, err_msg=name)
    if 'residuals' in reference:
        np.testing.assert_allclose(reference['residuals'], candidate['residuals'], rtol=1e-3, atol=1e-8, err_msg='certified residuals')
    for name in ('band_indices', 'solved_band_indices', 'retained_band_indices', 'remote_band_indices',
                 ...
                 'stencil_execution_order', 'reference_sample_index', 'build'):
    ...
    for name, info in reference['array_info'].items():
        ...
        if info['dimensions'][-2:] in (['retained_band', 'solved_band'], ['retained_band', 'retained_band'],
