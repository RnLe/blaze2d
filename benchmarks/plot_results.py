...
Creates:
1. Bar chart comparing single-core performance (MPB, Blaze2D)
2. Bar chart comparing multi-core performance (MPB Native, MPB Process, Blaze2D)
"""
...
COLORS = {
    "mpb": "#E74C3C",         # Red
    "blaze2d": "#3498DB",     # Blue
    "speedup": "#9B59B6",     # Purple (for misc)
}
...
def plot_time_comparison(ax, mode_data: dict, title: str):
    ...
    if core_mode == "multi":
        ...
        bars1 = ax.bar(x + offset, mpb_times, width, yerr=mpb_errs,
                       label='MPB (Native)', color=COLORS["mpb"], capsize=3, alpha=0.8)
        bars2 = ax.bar(x + offset + width, mpb_proc_times, width, yerr=mpb_proc_errs,
                       label='MPB (Process)', color="#8E44AD", capsize=3, alpha=0.8)
        bars3 = ax.bar(x + offset + 2*width, blaze_times, width, yerr=blaze_errs,
        ...
        bars3 = ax.bar(x + offset + width, blaze_times, width, yerr=blaze_errs,
                       label='Blaze2D', color=COLORS["blaze2d"], capsize=3, alpha=0.8)
...
def create_full_report(results: dict, output_dir: Path):
    """Create figure with timing comparison plots only."""
    ...
    # Create figure with subplots - timing plots only (1 row)
    if n_modes == 2:
        # 1x2 layout: single-core, multi-core timing only
...
def create_individual_plots(results: dict, output_dir: Path):
    """Create individual timing plots for each core mode."""
