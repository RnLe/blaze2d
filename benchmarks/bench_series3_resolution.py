#!/usr/bin/env python3
"""
Benchmark Series 3: Resolution Sweep on Square Lattice

This benchmark varies the resolution from 16 to 512 for both TM and TE
polarizations on a square lattice (Config A: ε=8.9 rods in air, r=0.2a).

Compares MPB and Blaze2D - single-core only.

Output: results/series3_resolution/
"""
...
RESOLUTION_VALUES = [16, 24, 32, 48, 64, 96, 128, 192]
...
NUM_ITERATIONS = 2  # Iterations per resolution value
...
# ============================================================================
BLAZE_TEMPLATE = """# Series 3: Square lattice, air bg, ε=8.9 rods, r=0.2a - {pol}
polarization = "{pol}"
...
def run_mpb_benchmark(resolution: int, polarization: str) -> float:
    """Run MPB benchmark for a single resolution value."""
    import meep as mp
    from meep import mpb
...
def run_blaze_benchmark(resolution: int, polarization: str, binary: Path) -> float:
    """Run Blaze2D benchmark for a single resolution value."""
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=1800,  # 30 min timeout for high resolutions
        )
        elapsed = time.perf_counter() - start
...
def run_series(output_dir: Path, quick: bool = False):
    """Run the full resolution sweep benchmark."""
    
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Parameters
    iterations = 1 if quick else NUM_ITERATIONS
    resolutions = [16, 32, 64] if quick else RESOLUTION_VALUES  # Subset for quick mode
    ...
    # Results storage
    results = {
        "series": "series3_resolution",
        "description": "Resolution sweep on square lattice (Config A)",
        ...
        "TM": {"mpb": [], "blaze": [], "resolution": []},
        "TE": {"mpb": [], "blaze": [], "resolution": []},
    }
    ...
    for pol in ["TM", "TE"]:
        ...
        for resolution in resolutions:
            ...
            print(f"  res = {resolution:3d}×{resolution:3d}: ", end="", flush=True)
            
            for it in range(iterations):
                # MPB
                if mpb_available:
                    t = run_mpb_benchmark(resolution, pol)
                    mpb_times.append(t)
                    print("M", end="", flush=True)
                
                # Blaze2D
                if blaze_available:
                    t = run_blaze_benchmark(resolution, pol, blaze_binary)
                    blaze_times.append(t)
                    print("B", end="", flush=True)
            
            # Store results
            results[pol]["resolution"].append(resolution)
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Series 3: Resolution Sweep")
    parser.add_argument("--output", type=str, default="results/series3_resolution",
                        help="Output directory")
    parser.add_argument("--quick", action="store_true",
                        help="Quick mode (fewer resolution values, 1 iteration)")
