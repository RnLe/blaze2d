...
Degenerate eigenvectors require subspace comparisons. Interface differences
represent discretization differences and must be checked under refinement.

\emph{Throughput}: mixed precision reduces field storage and changes the cost
of numerical operations. The recorded speedups depend on the workloads in
\cref{fig:blaze2d_benchmarks,fig:blaze2d_iterations_memory}. These measurements
alone do not establish a bandwidth limit or an unchanged eigenvalue trajectory.
Independent configurations can be scheduled concurrently for parameter studies.
